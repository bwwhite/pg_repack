...
static void repack_all_databases(const char *order_by, const char *copy_where);
static bool repack_one_database(const char *order_by, const char *copy_where, char *errbuf, size_t errsize);
...
main(int argc, char *argv[])
{
	...
	if (r_index.head || only_indexes)
	{
		...
		else
		{
			if (orderby)
				...
				ereport(WARNING, (errcode(EINVAL),
					errmsg("option -d (--copy-where) has no effect while repacking indexes")));
	...
	else
	{
		...
		if (alldb)
		{
			...
			repack_all_databases(orderby,copywhere);
		}
		else
		{
			if (!repack_one_database(orderby, copywhere, errbuf, sizeof(errbuf)))
				ereport(ERROR,
 ...
 */
static void
repack_all_databases(const char *orderby, const char *copywhere)
{
	PGresult   *result;
	int			i;
	...
	for (i = 0; i < PQntuples(result); i++)
	{
		...
		if (!dryrun)
		{
			ret = repack_one_database(orderby, copywhere, errbuf, sizeof(errbuf));
			if (!ret)
 ...
 */
static bool
repack_one_database(const char *orderby, const char *copywhere, char *errbuf, size_t errsize)
{
	bool					ret = false;
	PGresult			   *res = NULL;
	int						i;
	...
	for (i = 0; i < num; i++)
	{
		...
		if (copywhere)
		{
			...
			/* This will cause a bulk delete to happen for the specified table. */
                        ...
                        appendStringInfoString(&sql, copywhere);
...
pgut_help(bool details)
{
	...
	printf("  -q, --copy-where=QUERY    delete data during the COPY phase by using a WHERE clause\n");
